...
class FakeShareSession:
    ...
    def query(self, model):
        ...
        return FakeShareQuery(self.values)
...
def test_share_report_creates_token_and_reopens_same_report(monkeypatch):
    session = FakeShareSession()
    app.dependency_overrides[get_db] = lambda: session
    monkeypatch.setattr("backend.services.kids_share_store.create_share_token", lambda: "shared-report-token")
    ...
    create_response = client.post("/api/kids-experience/share", json=payload)

    try:
        assert create_response.status_code == 200
        create_payload = create_response.json()
        assert create_payload["token"] == "shared-report-token"

        read_response = client.get(f"/api/kids-experience/share/{create_payload['token']}")

        assert read_response.status_code == 200
        read_payload = read_response.json()
        assert read_payload["draft"]["participant_name"] == "민지"
        assert read_payload["result"]["personality_type"] == "상상력 발명가"
        assert read_payload["result"]["recommended_jobs"][0]["title"] == "로봇 발명가"
    finally:
        app.dependency_overrides.clear()


def test_share_report_returns_404_for_missing_token():
    session = FakeShareSession()
    app.dependency_overrides[get_db] = lambda: session

    try:
        response = client.get("/api/kids-experience/share/not-found-token")
